...
public class DemoGUI: MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(10 * dpiScale, offset, buttonWidth * dpiScale, 20 * dpiScale * 1.5f), "Load next scene"))
        {
            ...
            if (currentSceneID < 6) currentSceneID++;
        ...
        if (GUI.Button(new Rect(10*dpiScale, offset += 25 * dpiScale, buttonWidth*dpiScale, 20*dpiScale), water.DrawScreenSpaceWaterMesh ? "Draw mode: Screen Space" : "Draw mode : Mesh", customButton))
        {
            water.DrawScreenSpaceWaterMesh = !water.DrawScreenSpaceWaterMesh;
        }

        if (water.DrawScreenSpaceWaterMesh)
        {
            offset += 35 * dpiScale;
            ...
            water.ScreenSpaceWaterResolution = GUI.HorizontalSlider(new Rect(10*dpiScale, offset, buttonWidth*dpiScale, 15), water.ScreenSpaceWaterResolution, 0.25f, 1);
        }
