...
public static class KW_Extensions {
    ...
    public static RenderTexture ReinitializeRenderTexture(RenderTexture rt, int width, int height, int depth, RenderTextureFormat format, bool clearRT = false)
    {
        if (rt == null || rt.width != width || rt.height != height)
        {
            if (rt != null) rt.Release();
            rt = new RenderTexture(width, height, depth, format);
            if(clearRT) ClearRenderTexture(rt, Color.black);
        }
    ...
    {
        //var dirs = Directory.GetDirectories(Application.dataPath, "KW_BakedData", SearchOption.AllDirectories);
        //return dirs.Length != 0 ? Path.Combine(dirs[0], "Resources") : string.Empty;
        return Path.Combine(Application.streamingAssetsPath, "WaterSystemData");
    }
    ...
    public static async Task<Texture2D> ReadTextureFromFileAsync(string pathToFileWithoutExtension, bool linear = true, FilterMode filterMode = FilterMode.Bilinear, TextureWrapMode wrapMode = TextureWrapMode.Repeat)
    {
        ...
        if (!File.Exists(pathToFileWithoutExtension + ".gz")) return null;
            ...
            using (GZipStream gzip = new GZipStream(fileStream, CompressionMode.Decompress))
            {
                using (MemoryStream stream = new MemoryStream())
                {
                    ...
                    await stream.ReadAsync(rawTextureDataWithInfo, 0, rawTextureDataWithInfo.Length);
                    {
                        ...
                        tex = new Texture2D(width, height, format, false, linear);
