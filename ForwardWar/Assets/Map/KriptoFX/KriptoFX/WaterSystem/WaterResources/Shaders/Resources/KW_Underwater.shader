...
#include "KW_WaterHelpers.cginc"
	...
	sampler2D _CameraColorTexture_Blured;
	...
	//return mask;
		...
		float z = tex2D(_CameraDepthTexture, i.uv.xy);
		...
		float3 volumeScattering = tex2D(KW_VolumetricLight, i.uv.xy).rgb;
#else
		...
		half3 waterColorUnder = tex2D(_CameraColorTexture, lerp(i.uv.xy, i.uv.xy * 1.75, 0));
		half3 waterColorBellow = tex2D(_CameraColorTexture, i.uv.xy + normals);
