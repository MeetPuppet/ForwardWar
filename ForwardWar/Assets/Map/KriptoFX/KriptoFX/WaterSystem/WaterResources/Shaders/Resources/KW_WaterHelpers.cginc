...
float ComputeUnderwaterFresnel(float angle)
{
	angle = Pow5(angle);
	return saturate(angle * 4 - 3);
}
...
//}
//
inline half4 Tex2D_AA_HQ(sampler2D tex, float2 uv)
{
	...
	color += tex2D(tex, uv.xy + (1.0 / 5.5) * uv_dx + (4.0 / 5.5) * uv_dy);
	color += tex2D(tex, uv.xy + (4.0 / 5.5) * uv_dx + (5.0 / 5.5) * uv_dy);
	color += tex2D(tex, uv.xy + (3.0 / 5.5) * uv_dx + (1.0 / 5.5) * uv_dy);
	color += tex2D(tex, uv.xy + (5.0 / 5.5) * uv_dx + (-3.0 / 5.5) * uv_dy);

	color += tex2D(tex, uv.xy + (2.0 / 5.5) * uv_dx + (-4.0 / 5.5) * uv_dy);
	color += tex2D(tex, uv.xy + (-1.0 / 5.5) * uv_dx + (-2.0 / 5.5) * uv_dy);
	color += tex2D(tex, uv.xy + (-3.0 / 5.5) * uv_dx + (-5.0 / 5.5) * uv_dy);
	color += tex2D(tex, uv.xy + (-4.0 / 5.5) * uv_dx + (-1.0 / 5.5) * uv_dy);

	color += tex2D(tex, uv.xy + (-5.0 / 5.5) * uv_dx + (3.0 / 5.5) * uv_dy);
	color += tex2D(tex, uv.xy + (-2.0 / 5.5) * uv_dx + (2.0 / 5.5) * uv_dy);
...
inline half4 Tex2D_AA_LQ(sampler2D tex, float2 uv)
{
	half4 color = tex2D(tex, uv.xy);
	half lum = dot(color.xz, float3(0, 1, 0));
...
inline half4 SampleBicubic(sampler2D tex, float2 uv, float4 texelSize)
{
	...
	half4 sample0 = tex2Dlod(tex, float4(offset.xz, 0, 0));
	half4 sample1 = tex2Dlod(tex, float4(offset.yz, 0, 0));
	half4 sample2 = tex2Dlod(tex, float4(offset.xw, 0, 0));
	half4 sample3 = tex2Dlod(tex, float4(offset.yw, 0, 0));
...
inline half3 ComputeDisplaceUsingFlowMap(sampler2D displaceTex, sampler2D flowMapTex, half3  displace, float3 worldPos, float2 uv, float2 flowMapUV, float time)
{
	half2 flowMap = tex2Dlod(flowMapTex, float4(flowMapUV, 0, 0)).xy * 2 - 1;
	...
	half3 tex1 = tex2Dlod(displaceTex, float4(uv - 0.25 * flowMap * time1, 0, 0));
	half3 tex2 = tex2Dlod(displaceTex, float4(uv - 0.25 * flowMap * time2, 0, 0));
...
inline half3 ComputeNormalUsingFlowMap(sampler2D normTex, sampler2D flowMapTex, half3 normal, float3 worldPos, float2 uv, float2 flowMapUV, float time)
{
	half2 flowMap = tex2D(flowMapTex, flowMapUV).xy * 2 - 1;
	half blendMask = abs(flowMap.x) + abs(flowMap.y);
	...
	half3 tex1 = tex2D(normTex, uv - 0.25 * flowMap * time1);
	half3 tex2 = tex2D(normTex, uv - 0.25 * flowMap * time2);
...
{
	x = 1 - x;
	return 0.02 + 0.98 * Pow5(x) * x * x * x; //fresnel aproximation http://wiki.nuaj.net/images/thumb/1/16/Fresnel.jpg/800px-Fresnel.jpg
}
