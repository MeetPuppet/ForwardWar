...
{
    Properties
    {   _Color("Color", Color) = (0.95, 0.95, 0.95, 0.15)
        ...
        SubShader
        {
            ...
            #define FORWARD_BASE_PASS
                ...
                v2f_foam vert_foam(appdata_foam v)
                {
                    ...
                    float offset = (floor(idx / KW_VAT_Position_TexelSize.w)) * KW_VAT_Position_TexelSize.x;

                    float4 lookup = tex2Dlod(KW_VAT_RangeLookup, float4(frac((time * _FPS) * KW_VAT_RangeLookup_TexelSize.x), 0, 0, 0));

                    float offsetMin = min(lookup.y - KW_VAT_Position_TexelSize.x * 0.5, offset);
                    float4 uv1 = float4(frac(float2(offsetMin + lookup.x, height) + KW_VAT_Position_TexelSize.xy * 0.5), 0, 0);
                   ...
                   // texturePos1.xyz = GammaToLinearSpace(texturePos1.xyz);
                    ...
                    float offsetMin2 = min(lookup.w - KW_VAT_Position_TexelSize.x * 1.0, offset);
                    float4 uv2 = float4(frac(float2(offsetMin2 + lookup.z, height) + KW_VAT_Position_TexelSize.xy * 0.5), 0, 0);
                  ...
                  //  texturePos2.xyz = GammaToLinearSpace(texturePos2.xyz);
                   ...
                   // float interpolationMask = abs(texturePos1.z - texturePos2.z) > 0.15 ? 0 : 1;
                    if (length(texturePos1.rgb) > 0.001 && length(texturePos2.rgb) > 0.001)
                    {
                  ...
                  //  waterOffset += ComputeBeachWaveOffset(worldPos, shorelineData);
                    ...
                    half3 lightColor = saturate(dot(-KW_LightDir, float3(0, 1, 0))) * KW_LightDirColor;
                    o.color.rgb = clamp(lightColor + ShadeSH9(float4(0, 1, 0, 1)), 0, 0.95);
                    o.color.rgb = dot(o.color.rgb, 0.333) * 0.85 + o.color.rgb * 0.25;
